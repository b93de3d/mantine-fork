...
function Configurator({
  ...
  const { classes, cx } = Configurator_styles['default']();
  ...
  const items = componentProps.map((prop, index$1) => {
    const _a = prop, { name, data, type, capitalize, initialValue } = _a, others = __objRest(_a, ["name", "data", "type", "capitalize", "initialValue"]);
