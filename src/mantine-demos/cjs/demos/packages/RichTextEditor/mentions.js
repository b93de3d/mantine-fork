...
function Demo() {
  ...
  const mentions = useMemo(
    () => ({
      allowedChars: /^[A-Za-z\\s\xC5\xC4\xD6\xE5\xE4\xF6]*$/,
...
function Demo() {
  const [value, onChange] = React.useState("<p>Type @ or # to see mentions autocomplete</p>");
  const mentions2 = React.useMemo(() => ({
    allowedChars: /^[A-Za-z\sÅÄÖåäö]*$/,
    mentionDenotationChars: ["@", "#"],
    source: (searchTerm, renderList, mentionChar) => {
      const list = mentionChar === "@" ? people : tags;
      const includesSearchTerm = list.filter((item) => item.value.toLowerCase().includes(searchTerm.toLowerCase()));
      renderList(includesSearchTerm);
    }
  }), []);
